...
- **Gate/verification preconditions** — if the plan's success is *measured* by comparing against **live/deployed state** or a **test fixture** (a gate harness, re-gate, regression run, A/B against a baseline, quality/grounding score), does it have an explicit Step 0 that asserts **(a) deploy-parity** — every edge function/RPC under test is deployed *at or after* the last commit that touched it — and **(b) fixture-cleanliness** — the fixture clinic/data passes its contract (name set, owner/doctor-name resolvable, required evidence present)? A gate missing this can produce **misleading green** by silently testing stale code or dirty data — the 2026-06 agent-engine push lost ~4 days to a `trigger-ai-step` that predated its own deploy plus a wrong doctor-name path. **No preflight on a gate-style plan ⇒ CHANGES REQUIRED.** The repo helper is `scripts/_gate_preflight.cjs` → `assertGatePreconditions({ functions, fixtureClinics })`.
