...
- **env `SUPABASE_ACCESS_TOKEN`** (root: `cwd`) — step 7e deploys edge functions (SKILL.md:34) — *step 7e only - commit and push (steps 1-6) run without it*
- **env `VITE_SUPABASE_PROJECT_ID`** (root: `cwd`) — --project-ref for the functions deploy (SKILL.md:34) — *step 7e only - commit and push (steps 1-6) run without it*
<!-- END requires-block -->
