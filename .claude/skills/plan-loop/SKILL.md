...
- **skill `reviewer`** — the loop's whole mechanism - SKILL.md:139-142 briefs a subagent to invoke it — *every round; the documented Skill() path is refused, so the subagent falls back to reading reviewer/SKILL.md by hand*
