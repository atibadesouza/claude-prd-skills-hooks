...
requires:
  - path: scripts/athena_db.py
    root: factory
    kind: file
    reason: "Management-API transport used to create the tenant schema"
  - env: SUPABASE_MGMT_TOKEN
    root: factory
    reason: "provisioning a new tenant schema needs management scope"
---
## Requires
<!-- BEGIN requires-block — generated by scripts/skill_requires_check.py --sync-body — do not hand-edit -->
- **env `SUPABASE_MGMT_TOKEN`** (root: `factory`) — provisioning a new tenant schema needs management scope
- **path `scripts/athena_db.py`** (root: `factory`) — Management-API transport used to create the tenant schema
<!-- END requires-block -->
