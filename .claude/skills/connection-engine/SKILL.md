...
requires:
  - path: scripts/connengine.py
    root: factory
    kind: file
    reason: "the engine itself - every command in this skill shells to it"
  - env: SUPABASE_PROJECT_REF
    root: factory
    reason: "athena_db.py resolves it from the repo-root .env (athena_db.py:36)"
  - env: SUPABASE_MGMT_TOKEN
    root: factory
    reason: "Supabase Management-API transport for the athenaai schema"
---
## Requires
<!-- BEGIN requires-block — generated by scripts/skill_requires_check.py --sync-body — do not hand-edit -->
- **env `SUPABASE_MGMT_TOKEN`** (root: `factory`) — Supabase Management-API transport for the athenaai schema
- **env `SUPABASE_PROJECT_REF`** (root: `factory`) — athena_db.py resolves it from the repo-root .env (athena_db.py:36)
- **path `scripts/connengine.py`** (root: `factory`) — the engine itself - every command in this skill shells to it
<!-- END requires-block -->
